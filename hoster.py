...
@app.route('/')
def home():
    return "I AM PY HOSTING BOT BY MAX "

def run_flask():
  # Make sure to run on port provided by environment or default to 8000
...
# --- Configuration ---
TOKEN = '8108276726:AAGiygRoT2KxtsmRK0VejEe4F9_OdCqEx04' # Replace with your actual token
OWNER_ID = 8049295100 # Replace with your Owner ID
ADMIN_ID = 8049295100 # Replace with your Admin ID (can be same as Owner)
YOUR_USERNAME = 'p4glu' # Replace with your Telegram username (without the @)
UPDATE_CHANNEL = 'https://t.me/stuff_portal' # Replace with your update channel link
...
def attempt_install_pip(module_name, message):
    ...
    if package_name is None: 
        ...
        return False 
    try:
        # Use plain text for notification to avoid complex MarkdownV2 escaping requirements
        text = f"BOT SOURCE CODE @stuff_portal 🐍 Module {module_name} not found. Installing {package_name}..."
        ...
        if result.returncode == 0:
            ...
            bot.reply_to(message, f"BOT SOURCE CODE @p4glu ✅ Package {package_name} (for {module_name}) installed.")
            return True
        else:
...
def attempt_install_npm(module_name, user_folder, message):
    try:
        ...
        if result.returncode == 0:
            ...
            bot.reply_to(message, f"BOT SOURCE CODE @p4glu ✅ Node package `{module_name}` installed locally.", parse_mode='Markdown')
            return True
        else:
        ...
        if attempt == 1:
            ...
            try:
                ...
                if return_code != 0 and stderr:
                    match_py = re.search(r"(?:ModuleNotFoundError: No module named '|ImportError: cannot import name '.+' from ')(.+?)(?:')", stderr)
                    if match_py:
...
def _logic_send_welcome(message):
    ...
    if user_id not in active_users:
        ...
        try:
            # Escape underscore in username to prevent Markdown parsing errors
            escaped_username = (user_username or 'N/A').replace('_', '\\_')
            owner_notification = (f"🎉 New user!\n👤 Name: {user_name}\n✳️ User: @{escaped_username}\n"
                                  f"🆔 ID: `{user_id}`\n📝 Bio: {user_bio}")
